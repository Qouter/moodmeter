...
- **Started from a clean slate** (2026-06-26): wiped all `mood_entries` (255 demo rows) and **removed the `seedIfEmpty()` call from `App.tsx`'s load effect** so the app no longer auto-generates 21 days of fake data on an empty table — it boots empty for real. `seedIfEmpty` is still exported and wired to the manual "sembrar demo" button (`onSeed`), so demo data is now opt-in only. `user_settings` left untouched.
- **Mood trigger note** (2026-06-26): migration `add_note_to_mood_entries` added a nullable `note text` column to `mood_entries` for an optional short free-text on what the user thinks caused the mood ("¿Qué crees que lo provocó?"). `Entry.note` added; `loadEntries`/`addEntry` read/write it (empty string ⇄ `null`). Captured via an optional 2nd input in the `CheckInScreen` bottom sheet under "Una palabra" (the emotion word stays required, the note doesn't; max 120 chars). Shown as a muted, truncated second line under the word in `HistoryScreen`. Doesn't touch the bot or Insights. RLS unchanged (column-level access inherits the table policies).
...
- **Telegram bot — done end-to-end**: bot is `@moodmeter_thebot`. Three secrets in Vault: `telegram_bot_token`, `telegram_webhook_secret`, `telegram_cron_secret`. SECURITY DEFINER function `public.get_telegram_secrets()` (granted only to `service_role`) returns all three so Edge Functions can read Vault via `.rpc()` — PostgREST does not expose the `vault` schema directly, which was the bug that made the first webhook version silently swallow Telegram updates. `pg_cron` + `pg_net` enabled. `user_settings` extended with `telegram_link_token` (unique MM-XXXX-XXXX, indexed), `paused_until`, `last_ping_at`. (Briefly added `pending_mood` for an in-Telegram /mood entry flow, then dropped it: the user wants mood capture to live only in the web app — the bot is a reminder channel, not an entry surface.) UI in Ajustes auto-generates the link token on first load and shows it next to `@moodmeter_thebot`; connection state reads from `telegram_chat_id`. Two Edge Functions deployed with `verify_jwt: false` and custom secret auth: `telegram-webhook` (commands: `/start <token>`, `/pause [Nh|Nm|Nd]`, `/skip`, `/settings`; any other text replies with help + the app URL) and `telegram-pings` (cron-driven; ports `computeSchedule` from `src/lib/format.ts` into Deno; respects weekend mode, window, `paused_until`, and `last_ping_at`-today to avoid duplicates; due window is the last 30 min of a scheduled slot; on send delivers `🌿 ¿Cómo te sientes ahora?` plus an inline button to `APP_URL`). `pg_cron` job `telegram-pings-every-minute` (`* * * * *`) fires `net.http_post` to `telegram-pings` with the cron secret in `x-cron-secret`. Verified live: the 12:50 Madrid tick returned 200 with the right schedule, and a forced-slot manual non-dryRun trigger delivered the ping to Telegram.

- **Google Calendar — done (MVP)**: scope `https://www.googleapis.com/auth/calendar.readonly` (the broader one — `calendar.events.readonly` alone is rejected by `/users/me/calendarList` with PERMISSION_DENIED, even though it lets you read events from a known calendar id). Requested from the client in `signInWithGoogle` with `access_type: offline` so the session also carries `provider_refresh_token` (kept for a possible later server-side refresh path, unused now). `src/lib/calendar.ts` walks `calendarList` (minAccessRole=reader, not hidden, not deselected) and `Promise.allSettled`s the events endpoint of each calendar — that way a single broken share doesn't poison the rest, and shared/work calendars added to a personal account flow in alongside the primary. `InsightsScreen` keys its fetch effect to `session.provider_token` so a re-auth refreshes events without a page reload, and surfaces four states: loading, ready, needs-auth (with a "Conectar Google Calendar" button that re-runs `signInWithGoogle`), and error. In Google Cloud the OAuth Consent Screen needs the scope registered under "Data access" (sensitive) for it to actually be granted at consent time, and the app needs to be in Testing mode with the user listed as a test user — otherwise Google strips the scope silently and you end up with a token whose tokeninfo claims to have it but Calendar API still returns 403. `MOCK_CALENDAR` is no longer consumed but the constant + `CalendarEvent` type remain exported for the demo seed.

### Pending — implementation, in order

1. **Bot token rotation** (low-priority cleanup): the bot token was pasted in chat earlier. Rotate via @BotFather `/mybots → API Token → Revoke`, then update the Vault secret `telegram_bot_token`.
2. **Calendar token refresh** (nice-to-have): the Google access token in `session.provider_token` expires ~1h after sign-in and Supabase does not auto-refresh provider tokens. Today this surfaces as the "needs-auth" state. We already request `access_type: offline` so a `provider_refresh_token` is stored in the session — a future path is a small Edge Function that exchanges that refresh token for a fresh access token on demand, so the user never has to re-auth manually.
