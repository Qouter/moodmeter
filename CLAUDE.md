...
1. **Google Calendar**: extend the Supabase Google OAuth provider with scope `https://www.googleapis.com/auth/calendar.events.readonly`. Edge Function `gcal-events` reads day events via the user's stored access token, returns to the Insights screen to replace `MOCK_CALENDAR`.
2. **Bot token rotation** (low-priority cleanup): the bot token was pasted in chat earlier. Rotate via @BotFather `/mybots → API Token → Revoke`, then update the Vault secret `telegram_bot_token`.
