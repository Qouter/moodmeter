...
- **Telegram bot — partial**: bot is `@moodmeter_thebot` (created via @BotFather). Bot token + webhook secret stored in Supabase Vault as `telegram_bot_token` / `telegram_webhook_secret`. Extensions `pg_cron` + `pg_net` enabled. `user_settings` extended with `telegram_link_token` (one-time MM-XXXX-XXXX code, unique-indexed), `paused_until`, `last_ping_at`, `pending_mood` (jsonb for the two-step /mood conversation). UI in Ajustes auto-generates the link token on first load and shows it next to `@moodmeter_thebot`; connection state is driven by `telegram_chat_id`. Edge Function `telegram-webhook` deployed (verify_jwt off; auth via the `X-Telegram-Bot-Api-Secret-Token` header), handles `/start <token>`, `/mood` (asks for word → 4-quadrant inline keyboard → saves entry to `mood_entries`), `/pause [Nh|Nm|Nd]`, `/skip`, `/settings`. Webhook registered with Telegram (URL `https://wgvklmfizztewfkvjmsg.supabase.co/functions/v1/telegram-webhook`, `allowed_updates=['message','callback_query']`).

### Pending — implementation, in order

1. **Telegram bot — finish**: Edge Function `telegram-pings` (computes whose ping is due now using the same schedule logic as `src/lib/format.ts::computeSchedule`, posts via Telegram API, updates `last_ping_at`). `pg_cron` job running every minute that invokes it via `pg_net.http_post`. Schedule generation has to be ported to the edge function (TS in Deno) since pg_cron triggers via HTTP — the JS algorithm in `src/lib/format.ts` is the source of truth. Bot token rotation pending (token was pasted in chat earlier).
